...
class KD_model(nn.Module):
    def __init__(self, conf, task, num_labels):
        ...
        if task in conf:
            checkpoint = torch.load(conf[task], map_location=torch.device('cpu'))
...
class RAIL_c_model(KD_model):
    def __init__(self, conf, task, num_labels):
        ...
        self.sent_pool = conf['sent_pool'] if 'sent_pool' in conf else False
    ...
    def RAIL_c_loss(self, soft_label, outputs):
        ...
        hs_s = outputs.hidden_states[1:]
        if self.sent_pool:
            h_bar_t = list(map(self.sent_mean_pooling, hidden_t))
            h_bar_s = list(map(self.sent_mean_pooling, hidden_s))
        else:
    ...
    def sent_mean_pooling(self, hidden_states):
        # do not use [PAD]
        hidden_states[self.attention_mask==0] = torch.zeros(hidden_states.shape[2])
        # shape (batch_size, sequence_length, hidden_size) -> (batch_size, hidden_size)
        return torch.sum(hidden_states, dim=1)/sum(self.attention_mask, dim=1)
...
class CatILD_model(KD_model):
    ...
    def sent_mean_pooling(self, hidden_states, attention_mask):
        # do not use [PAD]
        hidden_states[attention_mask==0] = torch.zeros(hidden_states.shape[2])
        # shape (batch_size, sequence_length, hidden_size) -> (batch_size, hidden_size)
        return torch.sum(hidden_states, dim=1)/sum(attention_mask, dim=1)
...
class MATE_model(KD_model):
    ...
    def bart_inps(
        ...
        teacher_inp = torch.matmul(prediction_scores, self.teacher.model.encoder.embed_tokens.weight) * mask_permuted.unsqueeze(-1)
        student_inp = torch.matmul(prediction_scores, self.student.model.encoder.embed_tokens.weight) * mask_permuted.unsqueeze(-1)
        teacher_inp = teacher_inp + (self.teacher.model.encoder.embed_tokens(input_ids) * (1 - mask_permuted.unsqueeze(-1)))
        student_inp = student_inp + (self.student.model.encoder.embed_tokens(input_ids) * (1 - mask_permuted.unsqueeze(-1)))

        # d_teacher_inp = self.shift_embeds(teacher_inp, self.teacher)
        # d_student_inp = self.shift_embeds(student_inp, self.student)

        d_prediction_scores = self.shift_prediction_scores(prediction_scores, self.generator)
        d_teacher_inp = torch.matmul(d_prediction_scores, self.teacher.model.decoder.embed_tokens.weight) * mask_permuted.unsqueeze(-1)
        d_student_inp = torch.matmul(d_prediction_scores, self.student.model.decoder.embed_tokens.weight) * mask_permuted.unsqueeze(-1)
        d_teacher_inp = teacher_inp + (self.teacher.model.decoder.embed_tokens(input_ids) * (1 - mask_permuted.unsqueeze(-1)))
        d_student_inp = student_inp + (self.student.model.decoder.embed_tokens(input_ids) * (1 - mask_permuted.unsqueeze(-1)))

        return (teacher_inp,  d_teacher_inp), (student_inp, d_student_inp)

    def shift_prediction_scores(self, prediction_scores, model):
        shifted = prediction_scores.new_zeros(prediction_scores.shape)
        shifted[:, 1:] = prediction_scores[:, :-1]
        shifted[:, 0, model.config.decoder_start_token_id] = 1
...
class Bart_CILDA_model(MATE_model):
    ...
    def CRD_loss(
        ...
        for h in hidden_t:
            ...
            h_bar_t_c = torch.cat([h_bar_t_c, h[eos_mask, :]], dim=1)
        for h in hidden_s:
            h_bar_s_c = torch.cat([h_bar_s_c, h[eos_mask, :]], dim=1)
    ...
    def forward(self,
        ...
        if self.adv:
            ...
            lcrdp_e = self.CRD_loss(outputs_t_p.encoder_hidden_states[1:], outputs_s_p.encoder_hidden_states[1:], self.We_t, self.We_s, [self.teacher.config.encoder_layers, self.student.config.encoder_layers], eos_mask)
            lcrdp_d = self.CRD_loss(outputs_t_p.decoder_hidden_states[1:], outputs_s_p.decoder_hidden_states[1:], self.Wd_t, self.Wd_s, [self.teacher.config.decoder_layers, self.student.config.decoder_layers], eos_mask)
